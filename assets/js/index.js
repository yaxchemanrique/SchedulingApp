...
const renderTimeTags = () =>{
    ...
    for (let i = 0; i < timesResponseJsonArray.length; i++) {
        if (selectedDateWDashes === timesResponseJsonArray[i].date) {
            ...
            const isAvailable = timesResponseJsonArray[i].availability;
            let timetagInnerHTML = '';
            if(isAvailable){
                timetagInnerHTML = 
                `<input type="radio" id="time${i}" name="appt-time" value="time${i}">
                <label for="time${i}" id="time${i}-label" class="time-tag-label">
                    <div class="time-icon">
                ...
                timetagInnerHTML = 
                `<input type="radio" id="time${i}" name="appt-time" value="time${i}">
                <label for="time${i}" id="time${i}-label" class="disabled time-tag-label">
                    <div class="time-icon">
                        <svg class="plus-svg" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M12 21V12M12 12V3M12 12H22M12 12H2" stroke="white" stroke-width="2.5" stroke-linecap="round"/>
                        </svg>
